...
const DEF_WALL_STYLE = {
  material: Cesium.Color.GREEN.withAlpha(0.5)
}

const DEF_CYLINDER_STYLE = {
  topRadius: 0.0,
  bottomRadius: 1000.0,
  material: Cesium.Color.GREEN.withAlpha(0.5)
}
...
class Track {
  constructor(positions, duration, callback, options) {
    ...
    this._cylinder = new Cesium.Entity({
      show: false,
      position: new Cesium.CallbackProperty(() => {
        let position = undefined
        let lastPosition = this._pathPositions[this._pathPositions.length - 1]
        if (lastPosition) {
          let cartographic = Cesium.Cartographic.fromCartesian(lastPosition)
      ...
      cylinder: {
        length: new Cesium.CallbackProperty(() => {
          ...
          if (lastPosition) {
            let cartographic = Cesium.Cartographic.fromCartesian(lastPosition)
            this._cylinderLength = cartographic.height
          }
  ...
  _onAdd(controller) {
    ...
    this._controller.delegate.add(this._wall)
    this._controller.delegate.add(this._cylinder)
  ...
  _onRemove() {
    ...
    this._controller.delegate.remove(this._wall)
    this._controller.delegate.remove(this._cylinder)
    this._viewed = false
    this._startTime = undefined
    ...
    if (Cesium.JulianDate.lessThanOrEquals(now, this._endTime)) {
      ...
      if (time) {
        ...
        if (timeDiff >= 0 && timeDiff <= 1) {
          ...
          // 是否最后一个点
          let isLast = this._positionIndex + 1 === this._positions.length
          // 最后一个点时，清理跟随视角的实体，避免渲染报错
          isLast &&
            viewer.delegate.trackedEntity &&
            (viewer.delegate.trackedEntity = undefined)
          // 调用回调函数，返回当前位置和是否最后一个点位
          this._callback && this._callback(position, isLast)
          this._positionIndex++
        }
      }
    }
   ...
   * @returns {Track}
   */
  setWall(visible, style = {}) {
    this._wall.show = !!visible
    Util.merge(this._wall.wall, DEF_WALL_STYLE, style)
    return this
  }
   ...
   * @returns {Track}
   */
  setCylinder(visible, style = {}) {
    this._cylinder.show = !!visible
    Util.merge(this._cylinder.cylinder, DEF_CYLINDER_STYLE, style)
    return this
  }
}
