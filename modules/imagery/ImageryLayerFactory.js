...
import SingleTileExImageryProvider from './provider/SingleTileExImageryProvider'
...
class ImageryLayerFactory {
  ...
  /**
   * 创建单张图片加载图层，支持webMercator坐标系
   * @param options
   * @returns {SingleTileExImageryProvider|SingleTileExImageryProvider}
   */
  static createSingleTileExImageryLayer(options) {
    return new SingleTileExImageryProvider(options)
  }
  /**
  ...
  static createImageryLayer(type, options) {
    ...
    switch (type) {
      ...
      case ImageryType.SINGLE_TILE:
        ...
        break
      case ImageryType.SINGLE_TILE_EX:
        imageryLayer = this.createSingleTileExImageryLayer(options)
        break
