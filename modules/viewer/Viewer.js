...
class Viewer {
  ...
  _addLayer(layer) {
    ...
    if (!Object(this._layerCache[layer.type]).hasOwnProperty(layer.id)) {
      // 自定义图层继承Layer类，通过发送事件加载
      if (layer.fire) {
        layer.fire(LayerEventType.ADD, this)
      } else {
        // TODO: 默认作为imageryProvider加载，后续需要根据类型判断，避免类型不匹配导致的加载错误
        this.imageryLayers.addImageryProvider(layer)
      }
  ...
  _removeLayer(layer) {
    if (Object(this._layerCache[layer.type]).hasOwnProperty(layer.id)) {
      layer.fire && layer.fire(LayerEventType.REMOVE, this)
